# Front-End Görevi: Ürün Güncelleme Talebi Detay Sayfası (Tedarikçi Görünümü)

## Hazır Olma Tanımı:

**API Endpoint hazır:** ✅ Evet - `GET /api/requests/{id}`, `PUT /api/requests/{id}/cancel`
...
**Kullanıcı Hikayesi:** Bir **tedarikçi** olarak, **gönderdiğim ürün güncelleme talebinin detaylarını görüntülemek, mevcut ürün bilgileriyle karşılaştırmak ve gerekirse talebi iptal etmek** istiyorum, böylece **ürün güncelleme sürecimi takip edip yönetebilirim**.

**Özellik/Epik:** Ürün Talep Yönetim Sistemi / Tedarikçi Paneli

**Öncelik:** Yüksek

> **Önemli:** Ürün güncelleme taleplerinde **revizyon akışı yoktur** (`toBeRevised` statüsü kapalıdır). Talepler yalnızca **Kabul Edilir**, **Reddedilir** veya tedarikçi tarafından **İptal Edilir**.
...
| **Güncelleme Tarihi** | Sadece `updatedAt` varsa gösterilir — düzenleme ikonu ile |
...
| **Görsel Düzeni** | Kare görseller, 2 sütunlu grid |
...
| Buton | Stil | Davranış |
|-------|------|----------|
| **Yeni Talep Oluştur** | `bg-blue-600 text-white` + `fa-plus` ikonu | Yeni ürün güncelleme talebi sayfasına yönlendirir |

> **Not:** Ürün güncelleme taleplerinde revizyon yoktur, dolayısıyla "Düzenle" ve "Tekrar Gönder" butonları bu sayfada **bulunmaz**.

### 3.5 İptal Onay Modalı
...
- **Yeni Talep Oluştur:** Yeni güncelleme talebi sayfasına yönlendirir (rejected durumunda)
...
> **Not:** `approved` ve `rejected` geçişleri admin tarafından yapılır, tedarikçi bu aksiyonları yapamaz. Ürün güncelleme taleplerinde revizyon akışı yoktur.

**Sayfa UI Durumları:**

| Talep Durumu | Aksiyon Butonları | Red Gerekçesi | Karşılaştırma | Görüntüleme |
|-------------|-------------------|---------------|---------------|-------------|
| `submitted` | İptal Et | Gizli | Gösterilir | ✅ Var |
| `rejected` | Yeni Talep Oluştur | Gösterilir | Gösterilir | ✅ Var |
...
- **Toast Bildirimi:** İptal sonrası bilgi mesajı
...
| İptal Et sonrası | `#products/urun-guncelleme` |
| Yeni Talep Oluştur | Yeni güncelleme talebi sayfası |
...
- Sayfa başlığı: "Ürün Güncelleme Talebi #{id} - Tedarikçi Paneli"
...
- [ ] Revizyon ile ilgili butonlar **hiçbir durumda gösterilmiyor**
...
**Talep Yönetimi Kuralları (Ürün Güncelleme - Supplier):**
